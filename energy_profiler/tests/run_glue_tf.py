...
class GlueDataTrainingArguments:
    ...
    data_dir: Optional[str] = field(default='./tensorflow_datasets/', metadata={"help": "The input/output data dir for TFDS."})
...
def main(args):
    ...
    old_task_name = deepcopy(data_args.task_name)
    ...
    if data_args.task_name is not None:
        metric = load_metric("glue", old_task_name)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        preds = np.squeeze(preds) if output_mode == "regression" else np.argmax(preds, axis=1)
        if data_args.task_name is not None:
