...
def main(args):
    ...
    if len(args) == 1 and args.endswith(".json"):
    ...
    # Training
    if training_args.do_train:
        # TODO: implement training on Intel NCS2
        raise NotImplementedError('Training not supported on Intel NCS yet.')

    # Evaluation
    if training_args.do_eval:
        logger.info("*** Evaluate ***")

        # Instantiate the Intel NCS2
        ie = IECore()
        net = ie.read_network(model=model_args.model_name_or_path)

        # Load the network onto device
        print('Loading network onto the Intel NCS device...')
        exec_net = ie.load_network(network=net, device_name='MYRIAD')

        batch_size = training_args.per_device_eval_batch_size

        print('Running inference...')
        for step in tqdm(range(int(np.floor(eval_dataset.num_rows / batch_size)) - 1)):
            input_data = {'input_ids': [], 'attention_mask': []}

            for i in range(batch_size * step, batch_size * (step + 1)):
                input_data['input_ids'].append(eval_dataset[i]['input_ids'])
                input_data['attention_mask'].append(eval_dataset[i]['attention_mask'])

            # Run inference for the created batch on the Intel NCS2
